...
/**
 * VM调用底层机制
 * 第一步：提取SingleClient.class字节码放入方法区，在常量池有一个DCLSingle的符号引用
 * (这个引用还没有指向真正类信息的地址)
 * 第二步：运行到 将DCLSingle.class字节码信息加载到内存，这时DCLSingle的符号变量指向
 * class类信息
 * 第三步：声明DCLSingle类型 single变量，指向DCLSingle对象
 * 第四步：在堆区开辟空间，实例化DCLSingle对象
 * 指令集乱序导致第三步和第四步的执顺序不确定，single指向了DCLSingle对象的内存空间，由
 * 于指令集乱序的原因，此时DCLSingle对象可能还没有创建，volatile关键字解决执行集乱序，
 * 被volatile关键字修饰的变量创建的时候不乱序
 ...
 *
 * 加载一个类的原因(四种)：new、反射（Class.from），加载子类之前会先加载子类，其他的都不会加载类
 *
